...
function envoyerEmail($destinataire, $sujet, $messageHTML) {
    ...
    try {
        ...
        $mail->setFrom('lyonnekevs@gmail.com', 'MyBanque');
...
if (isset($_POST['envoyer_email'])) {
    $numCompte = $_POST['numCompte'];  // Récupérer le numéro de compte entré par l'utilisateur

    // Récupérer les informations du client
    $stmt = $connexion->prepare("SELECT c.Nom, c.Prenoms, c.mail, p.num_pret, r.rest_paye, (p.montant_prete - r.rest_paye) AS montant_rendu
                             FROM client c 
                             JOIN preter p ON c.numCompte = p.numCompte
                             JOIN rendre r ON p.num_pret = r.num_pret
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($rest_paye > 0) {
            ...
            if (envoyerEmail($email, $sujet, $messageHTML)) {
                echo "<div class='alert alert-info'>L'email de notification a été envoyé avec succès.</div>";
            } else {
                echo "<div class='alert alert-danger'>Erreur lors de l'envoi de l'email.</div>";
            }
        } else {
            echo "<div class='alert alert-success'>Ce client a déjà tout payé. Aucun email envoyé.</div>";
        }
    } else {
...
<!-- Formulaire HTML pour entrer le numéro de compte -->
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
